class e extends HTMLElement{constructor(){super(),this.render()}render(){let e=this.attachShadow({mode:"open"}),t=document.createElement("button"),a=document.createElement("style");a.innerHTML=`
  ...
  `;let d=a.querySelector(".playAgainButton");return console.log(d),d?.addEventListener("click",()=>{e.goTo("/juego")}),t}}],s="/desafio-ppt";function l(){return location.host.includes("github.io")}n.init();const c=document.querySelector(".root");if(c){function d(e){if("string"==typeof e){let t=l()?s+e:e;history.pushState({},"",t),u(t)}else console.error("La ruta debe ser un string:",e)}function u(e){console.log("el handle route recibió una nueva ruta: ",e);let t=l()?e.replace(s,""):e;for(let e of r)if(e.path.test(t)){c.dataset.currentRoute=t;let a=e.component({goTo:d});c.innerHTML="",c.appendChild(a)}}"/"===location.pathname?d("/wellcome"):u(location.pathname),window.onpopstate=function(){u(location.pathname)}}else console.error("Error: No se encontró el elemento con la clase .root");
//# sourceMappingURL=ppt.95cfb00c.js.map
