class e extends HTMLElement{constructor(){super(),this.render()}render(){let e=this.attachShadow({mode:"open"}),t=document.createElement("button"),a=document.createElement("style");a.innerHTML=`
  ...
  `;let d=a.querySelector(".playAgainButton");return console.log(d),d?.addEventListener("click",()=>{e.goTo("/juego")}),t}}];n.init();const s=document.querySelector(".root");if(s){function l(e){if("string"==typeof e){let t=location.host.includes("github.io")?"/desafio-ppt"+e:e;history.pushState({},"",t),c(t)}else console.error("La ruta debe ser un string:",e)}function c(e){for(let t of(console.log("el handle route recibió una nueva ruta: ",e),r))if(t.path.test(e)){s.dataset.currentRoute=e;let a=t.component({goTo:l});s.innerHTML="",s.appendChild(a)}}"/"===location.pathname?l("/wellcome"):c(location.pathname),window.onpopstate=function(){c(location.pathname)}}else console.error("Error: No se encontró el elemento con la clase .root");
//# sourceMappingURL=ppt.6f99f40d.js.map
