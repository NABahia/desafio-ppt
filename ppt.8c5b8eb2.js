class e extends HTMLElement{constructor(){super(),this.render()}render(){let e=this.attachShadow({mode:"open"}),t=document.createElement("button"),a=document.createElement("style");a.innerHTML=`
  ...
  `;let s=r.querySelector(".counter"),l=r.querySelector("circle"),d=r.querySelector("hand-select"),c=3,u=()=>{if(l){let e=c/3*283;l.style.strokeDashoffset=e.toString()}setTimeout(()=>{s&&(s.textContent=c.toString())},900)};u();let p=setInterval(()=>{if(c--,u(),0===c||d?.shadowRoot?.querySelector(".active")){clearInterval(p);let l=d?.shadowRoot?.querySelector(".active");if(l){var i,s;let d={1:"piedra",2:"papel",3:"tijera"}[s=3,i=Math.ceil(i=1),Math.floor(Math.random()*((s=Math.floor(s))-i+1)+i)],c=l.getAttribute("data-type"),u={miJugada:c,pcJugada:d};if(u){n.setJugadaActual(u);let e=n.getState();n.agregarHistorial(e.juego),n.agregarResultado(e.juego),r.innerHTML="",r.innerHTML=`
    ...
    <style>
      * {
      ...
      .win {
        background-color: rgba(40, 167, 69, 0.7); /* \u{2705} Verde con 70% de opacidad */
        color: white;
      }

      .lose {
        background-color: rgba(220, 53, 69, 0.7); /* \u{2705} Rojo con 70% de opacidad */
        color: white;
      }

      .draw {
        background-color: rgba(255, 193, 7, 0.7); /* \u{2705} Amarillo con 70% de opacidad */
        color: black;
      }
  ...
  `;let c=a.querySelector(".playAgainButton");return console.log(c),c?.addEventListener("click",()=>{e.goTo("/juego")}),t}}];n.init();const s=document.querySelector(".root");if(s){function l(e){if("string"==typeof e){let t=location.host.includes("github.io")?"/desafio-ppt"+e:e;history.pushState({},"",t),d(t)}else console.error("La ruta debe ser un string:",e)}function d(e){for(let t of(console.log("el handle route recibió una nueva ruta: ",e),r))if(t.path.test(e)){s.dataset.currentRoute=e;let a=t.component({goTo:l});s.innerHTML="",s.appendChild(a)}}"/"===location.pathname?l("/wellcome"):d(location.pathname),window.onpopstate=function(){d(location.pathname)}}else console.error("Error: No se encontró el elemento con la clase .root");
//# sourceMappingURL=ppt.8c5b8eb2.js.map
